		...
		<section className="w-full h-fit px-5 lg:px-10 my-5">
			<div className="flex flex-col border-b border-gray-300">
				...
				<div className="group relative w-62 rounded-lg bg-white border border-gray-100 shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden">
				      {/* Image Container */}
				      <div className="flex relative overflow-hidden bg-gradient-to-br from-slate-50 to-stone-100 w-48 h-52">
				        <img
				          src={item.image}
				          alt={item.title}
				          className="flex align-self-center justify-self-center w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
				        />

				        {/* Top-left: Best Seller + Discount stacked */}
				        <div className="absolute top-3 left-3 flex flex-col gap-1.5">
				          {badge && (
				            <span className="inline-flex items-center gap-1 bg-amber-400 text-amber-900 text-[11px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide shadow-sm">
				          ...
				          {item?.discount > 0 && (
				            <span className="inline-block bg-red-500 text-white text-[11px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide shadow-sm">
				              -{item?.discount}% OFF
				            </span>
				          )}
				        </div>
